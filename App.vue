...
const handleFileUpload = async (event) => {
  ...
  try {
    ...
    originalSentence.value = firstSentenceMatch ? firstSentenceMatch[0].trim() : text.trim().split('\n')[0];
  } catch (error) {
...
const generateParaphrases = async () => {
  if (!originalSentence.value) {
    errorMessage.value = '请先上传文件并提取句子。';
    return;
  }
  isLoading.value = true;
  errorMessage.value = '';
  ...
  try {
    ...
    errorMessage.value = '调用 API 失败，请检查后端服务是否运行。';
...
<template>
  <div class="container">
    <header>
      <h1>句子改写 ✍️</h1>
      <p>上传一个包含句子的 .txt 或 .doc 文件，AI 将为您提供 20+ 种不同的表达方式。</p>
    </header>
